...
Fill-in template for the report of a paired before/after benchmark (`benchmarks/run_pair.sh` →
`analysis/analyze_pair.py` → `publish_report.sh`). Copy the skeleton, replace every
`{{placeholder}}`, delete the instructional `> guidance` blockquotes and bracketed `[optional: ...]`
instructions, and keep the verbatim footnote lines as-is. The cross-objective warning is report
content rather than guidance; keep it only when its include condition holds. The section order is
fixed; do not reorder.

Most of the tables and footnotes are emitted verbatim by `analyze_pair.py` into
`improvement_stats.md` — drop its generic H1, paste the objective and statistics tables plus their
footnotes, then write the prose (title, Summary, plot captions) by hand. Optional sections and
optional clauses carry a one-line include condition; drop them when the condition does not hold.
...
>    `MNIST.{{network}}_linf0.1_authors` on the baseline and candidate commits under identical
>    non-objective settings; name each side's adversarial-example objective (`closest` computes
>    exact minimum distortion; `feasibility` searches for any verified witness in the fixed budget);
>    the ratio distributions, scatter plots, and outcome-flip tables compare each sample's candidate
>    run with its own baseline run, while the absolute-runtime distributions summarize each side
>    separately; and a link to the published `pairs/{{slug}}/` folder, listing what it holds (raw
>    per-sample CSVs, dependency snapshots, stats, plots, and any extras such as a same-commit
>    control) — either on the `benchmark-reports` branch or pinned to the report commit SHA (the
>    pinned form is stale-proof, matching C7's image URLs).
> 3. Baseline/candidate bullet (see convention C5 below).
> 4. The objective table emitted by `analyze_pair.py`; when the objectives differ, keep its exact
>    cross-objective warning immediately after the table (C11).
> 5. Reproduction bullet: the exact paired-run command, including explicit `--base-objective` and
>    `--candidate-objective` values rather than either commit's default; Julia `{{version}}` (read
>    from the run log) single-threaded, HiGHS named and glossed as "an open-source LP/MIP solver",
...
- **Baseline** `{{branch-or-label}}` `{{sha}}` [optional: provenance, e.g. "(post-#209 master, this
  PR's base)"]; **candidate** `{{branch-or-label — optional}}` `{{sha}}` [optional: provenance, e.g.
  "(benchmarked commit; `src/` byte-identical to head `{{sha}}`)"].

| run                                           | adversarial-example objective |
| --------------------------------------------- | ----------------------------- |
| {{baseline-label}} [{{baseline-objective}}]   | `{{baseline-objective}}`      |
| {{candidate-label}} [{{candidate-objective}}] | `{{candidate-objective}}`     |

<!-- keep only when the baseline and candidate objectives differ -->

> **Cross-objective comparison.** These runs use different solve goals. Interpret the timings as the
> performance tradeoff between exact distortion and a feasibility solve.

- Command:
  `benchmarks/run_pair.sh --base {{base-sha}} --candidate {{candidate-sha}} --out {{output-dir}} --samples {{1:N}} --tightening {{lp|mip|interval_arithmetic}} --main-time-limit {{120}} --norm-order {{Inf}} --base-objective {{closest|feasibility}} --candidate-objective {{closest|feasibility}}`.
  Julia `{{version}}`, single-threaded; HiGHS (an open-source LP/MIP solver) for all solves;
  sequential runs on a local WSL2 workstation [optional: with unrelated background jobs holding
  ~{{core-count}} cores throughout both sides], {{dependency-provenance}}. Absolute times are not
  comparable to the CI-hosted `benchmark-results` series.

[optional: italic report-scope note, e.g. _The paired-analysis tooling post-dates the merge, which
is why this report is retroactive._]
...
>
> - Objective context: name the shared objective once, or, for different objectives, frame the
>   timings as a performance tradeoff between exact distortion and a feasibility solve rather than
>   as a same-work speedup.
...
> magnitude of the shift, position relative to the `y = x` diagonal), not restating table numbers. A
...
> - `absolute_calls_ecdf.png` — `Absolute bound-call distributions` / `calls ECDF` _(optional; calls
>   series only)_
> - `calls_scatter.png` — `Paired bound-call scatter` / `calls scatter` _(optional; calls series
>   only)_
>
...
> reused verbatim everywhere. Use the `n` emitted for each row; the counts can differ by series
> because eligibility is evaluated per series (C3). `improved` / `regressed` are whole-percent
> shares. Include the `Bound solver calls` row only when the instrumentation columns exist.

| series                   |           n |     min |     p10 |     p25 |     median |     p75 |     p90 |     max | improved | regressed |
| ------------------------ | ----------: | ------: | ------: | ------: | ---------: | ------: | ------: | ------: | -------: | --------: |
| Build + bound tightening | {{n-build}} | {{min}} | {{p10}} | {{p25}} | {{median}} | {{p75}} | {{p90}} | {{max}} |   {{x}}% |    {{x}}% |
| Main solve time          |  {{n-main}} |       … |       … |       … |          … |       … |       … |       … |   {{x}}% |    {{x}}% |
| Total end-to-end time    | {{n-total}} |       … |       … |       … |          … |       … |       … |       … |   {{x}}% |    {{x}}% |
| Bound solver calls       | {{n-calls}} |       … |       … |       … |          … |       … |       … |       … |   {{x}}% |    {{x}}% |

- `ratio` = candidate ÷ baseline, < 1 = candidate faster; `improved` counts ratio below 0.99,
  `regressed` counts ratio above 1.01, samples within the ±1% band count as unchanged. [optional:
  when whole-percent rounding hides a visible outlier — a `max` above 1.01 next to `regressed` 0%
  (or below 0.99 next to `improved` 0%): "and shares round to whole percent ({{series}}'s lone
  regression, sample {{id}}, the `max` entry, rounds to 0%)"]
- `build` = constructing the MIP model; `tightening` = the `{{tightening}}` bound-tightening pass;
  `main solve` = the final verification MIP.
- `total` = `build` + `tightening` + `main solve`.
- `bound solver calls` = count of HiGHS bound-tightening solves [optional: parenthetical when the
  count moves: ", the count this PR changes" — match it to the observed direction, increase or
  decrease; drop it when the count is flat, as in #222]. <!-- keep only with the calls series -->
...
> Paste the emitted table. Same series rows as above, same alignment. Cells carry units inline (`s`,
> `calls`); `net saved` cells print an explicit `+`/`−` sign; `pooled ratio` to 2 decimals;
...
- `top-10 concentration` = the 10 samples with the largest absolute change account for this share of
  the total absolute per-sample change (0–100%; higher = a few samples dominate). [optional: when
  every paired value in a series is unchanged: "{{Series}} has zero total absolute per-sample
  movement, so its concentration is reported as 0%." Equal aggregate totals alone do not establish
  this condition.]
...
> / bound-tightening subtotals from the analyzer's dedicated timers). Bridge sentence first — or, as
> in the #222 report, as the trailing paragraph of the Aggregate section above: these timers split
...
- `Non-solver work within bound tightening` = bound-tightening time minus HiGHS wall time (interval
  propagation, certificate handling, bound-loop work for ReLU-scoped bounds).
- `Other formulation work` = formulation time minus bound-tightening time (target/objective
  construction, optimizer setup, non-solver work for bounds outside a ReLU layer).
- `Other work inside the sample call` = sample time minus formulation and final-solve wall time.

[optional: interpretive prose about where certificate work lands, why the rows moved, and any
measurement caveat]

#### Related timing views

> Optional — pairs with Component timings. Lead: "These rows use separate or overlapping timing
> views; do not add them to the table above." The `relationship` column states each row as a
> formula. After the table, add interpretive prose when it connects the result to an issue or design
> point (as with the issue #211 tie-in in the #222 report).
...
> under `### Solve status (all samples)`, `### Solve-status changes`, and
> `### Semantic-outcome changes`; consolidate them under this one H3 with the two label lines.) Drop
> the `Semantic outcome:` table when no semantic outcome changed (write `_None._`).

| status                        | {{baseline-label}} `{{short-sha}}` [{{baseline-objective}}] | {{candidate-label}} `{{short-sha}}` [{{candidate-objective}}] |
| ----------------------------- | ----------------------------------------------------------: | ------------------------------------------------------------: |
| INFEASIBLE                    |                                                       {{b}} |                                                         {{c}} |
| OPTIMAL                       |                                                       {{b}} |                                                         {{c}} |
...
- **C1 — Ratio direction.** `ratio` = candidate ÷ baseline; < 1 = candidate faster. `pooled ratio` =
  candidate total ÷ baseline total (the aggregate counterpart to the per-sample median). Ratios are
  always candidate-over-baseline.
- **C2 — ±1% unchanged band.** `improved` counts samples with ratio below 0.99, `regressed` counts
  ratio above 1.01; samples inside the ±1% band count as unchanged. Shares are whole-percent, so a
  lone outlier can round into a 0% share — note it in the per-sample footnote when the `max`/`min`
  makes it visible.
- **C3 — Series eligibility.** The analyzer first inner-joins the sample IDs and removes inputs
  skipped on either side. For each series, it then drops rows with a missing baseline or candidate
  value and rows whose baseline is not strictly positive. The emitted `n` and aggregate totals use
  that series-specific set, so `n` can differ between rows.
- **C4 — Absolute-times caveat.** Local sequential WSL2 workstation run; absolute times are not
  comparable to the CI-hosted `benchmark-results` series. Disclose any machine contention
  (concurrent load) that held during the run in the reproduction bullet.
- **C5 — Baseline/candidate identification.** Bold **Baseline** and **candidate** labels; commits as
  `` `{{branch-or-label}}` `{{sha}}` ``. The branch or label is optional — drop it when the
  benchmarked commit has none (e.g. a detached tip) — and the SHA may be short or full. Add a
  provenance note where it matters (base PR, byte-identical `src/`, current head not
  re-benchmarked).
...
- **C9 — Backticked identifiers.** Column/metric names, solve statuses, and semantic-outcome labels
  are code spans; transitions use `` `A` → `B` ``.
- **C10 — Table alignment.** Leading label column left-aligned; every numeric/count column
  right-aligned (`--:`). Units inline in cells; `net saved` carries an explicit sign; negatives use
  a Unicode minus. In the Component timings table, bold marks subtotal/parent rows only.
- **C11 — Objective context.** Paste the analyzer's objective table into the preamble, including the
  objective suffix on each run label. Keep the emitted cross-objective warning only when the two
  objectives differ, and frame the comparison as a solve-goal tradeoff. Reproduction commands must
  pass both side-specific objective flags explicitly; do not rely on commit defaults, which differ
  across benchmark versions. Historical output without objective metadata means `closest`.
