...
- Command: `benchmarks/benchmark_wk17a_first100.jl --samples <1:N> --tightening <lp|mip|interval_arithmetic> --main-time-limit <120> --norm-order <Inf>`. Julia `<version>`, single-threaded; HiGHS (an open-source LP/MIP solver) for all solves; sequential runs on a local WSL2 workstation<optional: , with unrelated background jobs holding ~<3> cores throughout both sides>, <dependency-snapshot hash `<sha256>` | identical dependency snapshots>. Absolute times are not comparable to the CI-hosted `benchmark-results` series.
