...
The first table is additive. Before rounding, rows with the same parent subtotal add exactly to
that parent, and indentation shows the same hierarchy.
...
“Non-solver work within bound tightening” is the bound-tightening subtotal minus HiGHS wall time.
“Other formulation work” is formulation minus bound tightening. “Other work inside the sample
call” is the small remainder outside the formulation and final-solve timers.

### Related timing views

These rows should not be added to the decomposition above. Whole-run elapsed is an independent
clock around the benchmark process. Formulation excluding bound-solver time cuts across two
branches of the additive table.
